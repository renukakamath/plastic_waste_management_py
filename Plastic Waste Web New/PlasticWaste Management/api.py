...
def view_routes():
	data={}
	log_id=request.args['logid']
	q="SELECT * FROM `route_assign` INNER JOIN `routes` USING(`route_id`) INNER JOIN `agents` USING(`agent_id`) INNER JOIN `collection_request` USING(`route_id`)  WHERE `agent_id`=(SELECT `agent_id` FROM `agents` WHERE `log_id`='%s') GROUP BY route_id"%(log_id)

	# q="SELECT * FROM `route_assign` INNER JOIN `routes` USING(`route_id`) INNER JOIN `agents` USING(`agent_id`) INNER JOIN `collection_request` USING(`route_id`)  INNER JOIN `company` ON `company`.`com_id`=`agents`.`com_id` WHERE `agent_id`=(SELECT `agent_id` FROM `agents` WHERE `log_id`='%s') AND `request_status`='Assign to Company'"%(log_id)
	print(q)
	res=select(q)
	if res:
		data['status']="Success"
...
def user_view_collection_report():
	...
	q="SELECT * FROM `collection_request` WHERE `user_id`=(SELECT `user_id` FROM `users` WHERE `log_id`='%s') order by date desc"%(log_id)
	print(q)
	res=select(q)
